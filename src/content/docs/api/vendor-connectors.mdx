...
<CardGrid>
  ...
  <Card title="Services" icon="rocket">
    ...
    - **Secrets** — Enterprise secret synchronization (Vault to AWS)
...
Synchronize secrets from Vault to AWS with high performance. Built on the [SecretSync](/packages/secretssync) package:

```python
from vendor_connectors import SecretsConnector
...
if result.success:
    print(f"Synced {result.secrets_processed} secrets in {result.duration_ms}ms")
```
...
| `SECRETS_CONFIG` | Secrets | Path to pipeline.yaml |
